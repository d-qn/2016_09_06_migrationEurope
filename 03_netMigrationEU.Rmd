...
data$bins <- cut(data$values, breaks = c(-Inf, -10, -5, -2.5, 0, 2.5, 5, 10, Inf))
colors <- c('#ab3d3f','#c36a66','#d79591','#e9c0be','#eef6f6','#bed1d0','#8fabab','#618888','#336666')
...
map <- ggplot() + 
  geom_polygon_interactive(
    data = df, aes(x=long,y=lat,group=group,fill=bins, data_id = NUTS_ID, tooltip = tip)
  ) +
  coord_quickmap(expand = F) + xlim(c(-18, 50)) + ylim (c(33, 73)) + 
  bk_mapTheme() +
  scale_fill_manual(values = colors) + 
  geom_polygon(data = ct, aes(x=long,y=lat,group=group, id = group), 
               fill =NA, colour = "#8c8c8c", size = 0.1)
